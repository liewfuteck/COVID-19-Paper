...
out_asymptomatic.df$time= sg_data$date
out_asymptomatic.df$time[out_asymptomatic.df$total >= 1375][1]
out_asymptomatic.df$total[nrow(out_asymptomatic.df)] - 1375
...
total_asymptomatic_epsilon_035 = total_asymptomatic_1
...
plot(sg_data$date, total_asymptomatic_epsilon_022, col = "green", pch = "*", xlab = "Date", ylab = "Cumulative Number of COVID-19 cases", ylim = c(0,5000), lty = 1)
lines(sg_data$date, total_asymptomatic_epsilon_022, col = "green", lty = 2)

points(sg_data$date, total_asymptomatic_epsilon_026, col='blue', pch = "*", lty = 1)
lines(sg_data$date, total_asymptomatic_epsilon_026, col = 'blue', lty = 2)

points(sg_data$date, total_asymptomatic_epsilon_035, col='black', pch = "*", lty = 1)
lines(sg_data$date, total_asymptomatic_epsilon_035, col = 'black', lty = 2)

points(sg_data$date, sg_data$total_cases, col = 'red', pch = '*', lty = 1)
lines(sg_data$date, sg_data$total_cases, col = 'red', lty = 1)


legend("topleft", legend = c("\u03F5, r = 0.22", 
                             "\u03F5, r = 0.26",
                             "\u03F5, r = 0.35",
                             "Reported number of cases"),
       col = c("green", "blue", "black", "red"),
       lty = c(2,2,2,1), pch = c("*","*","*","*"), cex = 1)
...
# To plot for effective reproduction number for different epsilon values

p = 0.156
r = c(1/3.85,0.58,0.63,0.75)
...
r_1 = p * r[1] * beta_array / k + (1-p)*beta_array*(1/(gamma*b + c/gamma) + 1/(gamma*d + e/gamma))
r_2 = p * r[2] * beta_array / k + (1-p)*beta_array*(1/(gamma*b + c/gamma) + 1/(gamma*d + e/gamma))
r_3 = p * r[3] * beta_array / k + (1-p)*beta_array*(1/(gamma*b + c/gamma) + 1/(gamma*d + e/gamma))
r_4 = p * r[4] * beta_array / k + (1-p)*beta_array*(1/(gamma*b + c/gamma) + 1/(gamma*d + e/gamma))
...
plot(sg_data$date, r_1, col = "green", pch = "*", xlab = "Date", ylab = "Effective reproduction number", ylim = c(2.5,5), lty = 1)
lines(sg_data$date, r_1, col = "green", lty = 2)

points(sg_data$date, r_2, col='blue', pch = "*", lty = 1)
lines(sg_data$date, r_2, col = 'blue', lty = 2)

points(sg_data$date, r_3, col='black', pch = "*", lty = 1)
lines(sg_data$date, r_3, col = 'black', lty = 2)

points(sg_data$date, r_4, col='darkorchid', pch = "*", lty = 1)
lines(sg_data$date, r_4, col = 'darkorchid', lty = 2)

legend("topleft", legend = c("r = 0.26", 
                             "r = 0.58",
                             "r = 0.63",
                             "r = 0.75"),
       col = c("green", "blue", "black", "darkorchid"),
       lty = c(2,2,2,2), pch = c("*","*","*","*"), cex = 1)